...
class Importer(ABC):
	...
	def prune_import(self, file: str):
		if self.nonoly_filepath(file).endswith(self.asset_file_extensions) and not os.path.exists(
...
def import_folder(prompt: str, importer: type[Importer]):
    ...
    # TODO v3 fix res_folder
	importer(folder=res_folder, recur=recur, prune=prune, default=default, clear=clear).run()
...
def import_manifest(package: str, importer: type[Importer]):
	...
	for folder in folders:
        ...
        # TODO v3 fix res_folder
		importer(folder=res_folder['path'], recur=recur, prune=prune, default=default, clear=clear).run()
