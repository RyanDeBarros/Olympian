...
def params_constructor(poly_composite, name) -> str:
    ...
    def write_bordered_ngon_method() -> str:
        ...
        c += write_vec4_vector(poly_composite, '_method.ngon_base.fill_colors', 'fill colors', 4)
        c += write_vec4_vector(poly_composite, '_method.ngon_base.border_colors', 'border colors', 4)
        if 'border width' in poly_composite:
            c += f"\t\t\t\t_method.ngon_base.border_width = (float){poly_composite['border width']};\n"
        if 'border pivot' in poly_composite:
            if poly_composite['border pivot'] is float:
                c += f"\t\t\t\t_method.ngon_base.border_pivot = (float){poly_composite['border pivot']};\n"
