...
// TODO v9.3 all of these are compositions of widget components. It's almost the same as sub-widgets, except that no properties are added, just checked. Add a boolean to bound_widget config that determines whether it is primary (add property) or composite (check property)
...
imtk::item_result imtk::w::bound_widget<imtk::edit_session<oly::editor::Rect>>::draw_impl()
{
	id_scope scope(&data);
	imtk::item_result result;
	...
	{
		imtk::prop::grid::subproperty_scope s;
		result |= DrawFloatComponent("x1", data.buffer().x1, config.cfg_x1.min, config.cfg_x1.max, config.cfg_x1.step, config.cfg_x1.step_fast, config.cfg_x1.format, config.cfg_x1.flags);
		imtk::controls::vertical_separator();
		result |= DrawFloatComponent("x2", data.buffer().x2, config.cfg_x2.min, config.cfg_x2.max, config.cfg_x2.step, config.cfg_x2.step_fast, config.cfg_x2.format, config.cfg_x2.flags);
		imtk::controls::vertical_separator();
		result |= DrawFloatComponent("y1", data.buffer().y1, config.cfg_y1.min, config.cfg_y1.max, config.cfg_y1.step, config.cfg_y1.step_fast, config.cfg_y1.format, config.cfg_y1.flags);
		imtk::controls::vertical_separator();
		result |= DrawFloatComponent("y2", data.buffer().y2, config.cfg_y2.min, config.cfg_y2.max, config.cfg_y2.step, config.cfg_y2.step_fast, config.cfg_y2.format, config.cfg_y2.flags);
	}

	data.post_edit(result.state);

	result.modified |= imtk::prop::grid::check_property(std::make_unique<imtk::prop::simple_view<imtk::edit_session<oly::editor::Rect>>>(data));
	return result;
}
...
imtk::item_result imtk::w::bound_widget<imtk::edit_session<oly::editor::UVRect>>::draw_impl()
{
	id_scope scope(&data);
	imtk::item_result result;
	...
	{
		imtk::prop::grid::subproperty_scope s;
		result |= DrawFloatComponent("x1", data.buffer().x1, 0.f, 1.f, config.cfg_x1.step, config.cfg_x1.step_fast, config.cfg_x1.format, config.cfg_x1.flags);
		imtk::controls::vertical_separator();
		result |= DrawFloatComponent("x2", data.buffer().x2, 0.f, 1.f, config.cfg_x2.step, config.cfg_x2.step_fast, config.cfg_x2.format, config.cfg_x2.flags);
		imtk::controls::vertical_separator();
		result |= DrawFloatComponent("y1", data.buffer().y1, 0.f, 1.f, config.cfg_y1.step, config.cfg_y1.step_fast, config.cfg_y1.format, config.cfg_y1.flags);
		imtk::controls::vertical_separator();
		result |= DrawFloatComponent("y2", data.buffer().y2, 0.f, 1.f, config.cfg_y2.step, config.cfg_y2.step_fast, config.cfg_y2.format, config.cfg_y2.flags);
	}

	data.post_edit(result.state);

	result.modified |= imtk::prop::grid::check_property(std::make_unique<imtk::prop::simple_view<imtk::edit_session<oly::editor::UVRect>>>(data));
	return result;
}
...
imtk::item_result imtk::w::bound_widget<imtk::edit_session<oly::editor::TopSidePadding>>::draw_impl()
{
	id_scope scope(&data);
	imtk::item_result result;
	...
	{
		imtk::prop::grid::subproperty_scope s;
		result |= DrawFloatComponent("left", data.buffer().left, config.cfg_left.min, config.cfg_left.max, config.cfg_left.step, config.cfg_left.step_fast, config.cfg_left.format, config.cfg_left.flags);
		imtk::controls::vertical_separator();
		result |= DrawFloatComponent("right", data.buffer().right, config.cfg_right.min, config.cfg_right.max, config.cfg_right.step, config.cfg_right.step_fast, config.cfg_right.format, config.cfg_right.flags);
		imtk::controls::vertical_separator();
		result |= DrawFloatComponent("top", data.buffer().top, config.cfg_top.min, config.cfg_top.max, config.cfg_top.step, config.cfg_top.step_fast, config.cfg_top.format, config.cfg_top.flags);
	}

	data.post_edit(result.state);

	result.modified |= imtk::prop::grid::check_property(std::make_unique<imtk::prop::simple_view<imtk::edit_session<oly::editor::TopSidePadding>>>(data));
	return result;
}
