...
namespace oly
{
	namespace input
	{
		static void load_key_binding(const toml::table& node, const std::string& id)
		{
			...
			if (!key)
				return;
			KeyBinding b{ .key = (int)key.value() };
			...
			context::platform().binding_context().register_signal(context::platform().signal_table().get(id), b);
		}
		...
		static void load_mouse_button_binding(const toml::table& node, const std::string& id)
		{
			...
			if (!button)
				return;
			MouseButtonBinding b{ .button = (int)button.value() };
			b.required_mods = (int)node["required mods"].value<int64_t>().value_or(0);
			b.forbidden_mods = (int)node["forbidden mods"].value<int64_t>().value_or(0);
		...
		static void load_gamepad_button_binding(const toml::table& node, const std::string& id)
		{
			...
			if (!button)
				return;
			GamepadButtonBinding b{ .button = (GamepadButton)(int)button.value() };
		...
		static void load_gamepad_axis_1d_binding(const toml::table& node, const std::string& id)
		{
			...
			if (!axis1d)
				return;
			GamepadAxis1DBinding b{ .axis = (GamepadAxis1D)(int)axis1d.value() };
		...
		static void load_gamepad_axis_2d_binding(const toml::table& node, const std::string& id)
		{
			...
			if (!axis2d)
				return;
			GamepadAxis2DBinding b{ .axis = (GamepadAxis2D)(int)axis2d.value() };
			b.deadzone = (float)node["deadzone"].value<double>().value_or(0.0);
		...
		{
			auto toml = assets::load_toml(signal_registry_filepath);
