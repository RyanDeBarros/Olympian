...
#define get_vec4_default(node, name, def) glm::vec4{ get_float_element_default(node[name].as_array(), 0, def[0]),\
 get_float_element_default(node[name].as_array(), 1, def[1]), get_float_element_default(node[name].as_array(), 2, def[2]), get_float_element_default(node[name].as_array(), 3, def[3]) }
		...
		random::bound::Function load_random_bound_function(const TOMLNode& node)
		{
			if (!node)
		...
		random::bound::Function2D load_random_bound_function_2d(const TOMLNode& node)
		{
			return { load_random_bound_function(node["x"]), load_random_bound_function(node["y"]) };
		}

		random::domain2d::Domain load_random_domain_2d(const TOMLNode& node)
		{
		...
		particles::EmitterParams load_particle_emitter_params(const TOMLNode& node)
		{
			...
			{
				static const auto load_acceleration = [](const TOMLNode& node) -> particles::acceleration::Function {
					std::string type = get_string(node, "type");
					if (type == "constant")
					{
			...
			if (auto color = node["color"])
			{
				...
				static const auto load_constant = [](const TOMLNode& node) {
					particles::color::Constant fn;
					fn.c = get_vec4_default(node, "c", glm::vec4(1.0f));
					return fn;
					};
				static const auto load_interp = [](const TOMLNode& node) {
				...
				else if (type == "piecewise")
				{
					...
					subfunctions->for_each([&fn](auto&& subfunction) {
						if constexpr (toml::is_table<decltype(subfunction)>)
						{
							...
							std::string type = get_string(subfunction, "type");
							if (type == "constant")
								subfn.fn = load_constant((TOMLNode)subfunction);
							else if (type == "interp")
								subfn.fn = load_interp((TOMLNode)subfunction);
		...
		particles::Emitter load_particle_emitter(const TOMLNode& node, const glm::vec4& projection_bounds)
		{
		...
		particles::ParticleSystem load_particle_system(const TOMLNode& node, const glm::vec4& projection_bounds)
		{
			...
			subemitter_nodes->for_each([&subemitters, &projection_bounds](auto&& emitter) {
				...
				{
					particles::Emitter subemitter = load_particle_emitter((TOMLNode)emitter, projection_bounds);
		...
		{
			return load_particle_system((const TOMLNode&)load_toml(file)["particle_system"], projection_bounds);
		}
