...
namespace oly::reg
{
	...
	{
		auto toml = load_toml(font_atlas_registry_file);
		...
		font_atlas_list->for_each([this](auto&& node) {
			if constexpr (toml::is_table<decltype(node)>)
			{
				...
				if (_name && _font_face && (_font_size_double || _font_size_int))
				{
					...
					parse_min_filter(node, "min filter", constructor.options.min_filter);
					parse_mag_filter(node, "mag filter", constructor.options.mag_filter);
					...
					if (_common_buffer_preset)
					{
						...
						if (common_buffer_preset == "common")
							constructor.common_buffer = rendering::glyphs::COMMON;
						else if (common_buffer_preset == "alpha numeric")
							constructor.common_buffer = rendering::glyphs::ALPHA_NUMERIC;
						else if (common_buffer_preset == "numeric")
							constructor.common_buffer = rendering::glyphs::NUMERIC;
						else if (common_buffer_preset == "alphabet")
							constructor.common_buffer = rendering::glyphs::ALPHABET;
						else if (common_buffer_preset == "alphabet lowercase")
							constructor.common_buffer = rendering::glyphs::ALPHABET_LOWERCASE;
						else if (common_buffer_preset == "alphabet uppercase")
							constructor.common_buffer = rendering::glyphs::ALPHABET_UPPERCASE;
					}
					...
					{
						auto_loaded.emplace(name, move_shared(create_font_atlas(name)));
	...
	rendering::FontAtlas FontAtlasRegistry::create_font_atlas(const std::string& name) const
	{
		...
		return rendering::FontAtlas(context::ref_font_face(constructor.font_face_name).lock(), constructor.options, constructor.common_buffer);
	}

	std::weak_ptr<rendering::FontAtlas> FontAtlasRegistry::ref_font_atlas(const std::string& name) const
	{
