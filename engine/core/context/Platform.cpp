...
namespace oly::context
{
	...
	static void load_modifier_base(input::ModifierBase& modifier, TOMLNode mnode)
	{
		...
		if (!reg::parse_float(mnode["multiplier"], modifier.multiplier.x))
			if (!reg::parse_vec(mnode["multiplier"], reinterpret_cast<glm::vec2&>(modifier.multiplier)))
				reg::parse_vec(mnode["multiplier"], modifier.multiplier);
	...
	{
		input::KeyBinding b;
		if (!reg::parse_int(node["key"], b.key))
			return;
		reg::parse_int(node["req_mods"], b.required_key_mods);
		reg::parse_int(node["ban_mods"], b.forbidden_key_mods);
	...
	static void load_mouse_button_binding(TOMLNode node, const std::string& id)
	{
		...
		if (!reg::parse_int(node["button"], b.button))
			return;
		reg::parse_int(node["req_mods"], b.required_button_mods);
		reg::parse_int(node["ban_mods"], b.forbidden_button_mods);
		b.modifier = load_modifier_0d(node);
	...
	{
		int button;
		if (!reg::parse_int(node["button"], button))
			return;
	...
	{
		int axis1d;
		if (!reg::parse_int(node["axis1d"], axis1d))
			return;
		input::GamepadAxis1DBinding b{ .axis = (input::GamepadAxis1D)axis1d };
		reg::parse_float(node["deadzone"], b.deadzone);
	...
	{
		int axis2d;
		if (!reg::parse_int(node["axis2d"], axis2d))
			return;
