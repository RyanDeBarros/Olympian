...
#define OLY_ENUM(EnumName, EntryMap)\
	...
	{\
		...
		EnumName(E value = E()) : value(value) { if (!NAMES_LUT.count((unsigned int)value)) throw Error(ErrorCode::DoesNotExist); }\
		EnumName(unsigned int value) : value((E)value) { if (!NAMES_LUT.count(value)) throw Error(ErrorCode::DoesNotExist); }\
