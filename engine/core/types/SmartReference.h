...
#define OLY_SMART_POOL_BASE(Class, Base) template<> struct oly::SmartPoolBase<Class> { using type = Base;\
	...
	namespace internal
	{
		...
		// TODO v12 multi-threading and thread safety: smart reference should have some kind of lock() similar to Issuer<T>::Handle.
		template<typename Object>
	...
	public:
		...
		// TODO v12 define macros that enables pointer data member for base() so it can be easily queried during debugging. Here and in other classes that abstract away references, like PublicIssuer.
