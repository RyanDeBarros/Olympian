...
namespace oly::rendering
{
	...
	std::vector<glm::vec2> StaticArrowExtension::get_all_points() const
	{
		if (dirty)
			clean();
		...
		if (can_draw_body)
			points.insert(points.end(), body.get_points().begin(), body.get_points().end());
		if (can_draw_head)
			points.insert(points.end(), head.get_points().begin(), head.get_points().end());
		return points;
	}

	void StaticArrowExtension::clean() const
	{
		dirty = false;
