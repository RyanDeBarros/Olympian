...
namespace oly::rendering
{
	...
	void SpriteAtlasResExtension::select_static_frame(GLuint frame)
	{
		anim_format.delay_seconds = 0.0f;
		select(frame);
	}

	void SpriteAtlasResExtension::uvs_changed() const
	{
		current_frame = -1;
	}

	void SpriteAtlasResExtension::setup_uniform(GLuint rows, GLuint cols, float delay_seconds, bool row_major, bool row_up)
	{
		atlas.clear();
	...
	void SpriteAtlasExtension::select_static_frame(GLuint frame)
	{
		anim_format.delay_seconds = 0.0f;
		select(frame);
	}

	void SpriteAtlasExtension::uvs_changed() const
	{
		current_frame = -1;
	}

	void SpriteAtlasExtension::setup_uniform(GLuint rows, GLuint cols, float delay_seconds, bool row_major, bool row_up)
	{
		atlas.clear();
		static const auto uv_rect = [](GLuint row, GLuint col, GLuint rows, GLuint cols) -> UVRect {
			return { {
